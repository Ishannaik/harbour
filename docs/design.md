...
- **10 sources** in 4 groups — Games: `gameshub` (HTML, the only code-running
  category → trusted repacker alone); Movies: `cinevault` (JSON, multi-host
  fallback), `vault-movies` (JSON, mirror-api), `reel-movies` (HTML), `torrent-hub`
  (HTML); TV: `showport` (RSS), `vault-tv` (JSON), `reel-tv` (HTML); Anime: `tsukibase`
  (RSS), `fansubs` (RSS).
...
pub struct TorrentResult {
    ...
    pub source: SourceId,    // e.g. SourceId::GamesHub
    pub magnet: String,

...
| --- | --- | --- | --- |
| fitgirl | FitGirl | Games | HTML scrape |
| yts | YTS | Movies | JSON API (yts.mx/.am/.rs fallback hosts) |
| tpb-movies | TPB | Movies | JSON API (apibay.org) |
| x1337-movies | 1337x | Movies | HTML scrape |
| eztv | EZTV | TV | RSS |
| tpb-tv | TPB | TV | JSON API (apibay.org) |
| x1337-tv | 1337x | TV | HTML scrape |
| nyaa | Nyaa | Anime | RSS |
| subsplease | SubsPlease | Anime | RSS |
| bittorrented | BitTorrented | Movies | HTML scrape |

Games are the only category that runs code → FitGirl alone (trusted repacker). If a source is down, search continues without it and the sidebar reports it offline.

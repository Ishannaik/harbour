...
- FitGirl HTML scraper (Games).
- YTS JSON API scraper with yts.mx / .am / .rs fallback hosts (Movies).
- TPB JSON API scraper on apibay.org for both `tpb-movies` and `tpb-tv` (Movies, TV).
- 1337x HTML scrapers for `x1337-movies` and `x1337-tv` (Movies, TV).
- EZTV RSS via quick-xml (TV).
- Nyaa RSS and SubsPlease RSS via quick-xml (Anime).
- BitTorrented HTML scraper (Movies).
...
| --- | --- | --- |
| cs.rin.ru source | Scraping feasibility unproven — Cloudflare challenge, forum/catalog structure; untrusted repackers violate the "FitGirl alone" trust rule until proven | Spike in phase 7 |
...
- After phase 1, three parallel workstreams, each as its own subagent(s):
  - **UI track**: Phase 2 (one subagent), then Phase 3 merged in (scrapers can be one subagent per source family — HTML: fitgirl/1337x/bittorrented; JSON: yts/tpb; RSS: eztv/nyaa/subsplease — fan out up to 3, with the `Source` trait + registry as the shared contract).

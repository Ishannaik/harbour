...
mod tests {
    ...
    impl Source for ReportingSource {
        ...
        fn reported_source_health(&self) -> HashMap<SourceId, (SourceStatus, u32)> {
            HashMap::from([
                (SourceId::CineVault, (SourceStatus::Online, 3)),
                (SourceId::TsukiBase, (SourceStatus::Empty, 0)),
                (SourceId::GamesHub, (SourceStatus::Offline, 0)),
            ])
        }
    }
    ...
    fn reported_site_health_merges_into_the_sidebar_after_a_search() {
        ...
        assert_eq!(
            app.state.search.source_health.get(&SourceId::CineVault),
            Some(&SourceStatus::Online),
            "an online site gets a live dot"
        );
        assert_eq!(
            app.state.search.source_health.get(&SourceId::TsukiBase),
            Some(&SourceStatus::Empty),
            "an empty site is reachable-but-nothing-matched"
        );
        assert_eq!(
            app.state.search.source_health.get(&SourceId::GamesHub),
            Some(&SourceStatus::Offline),
            "a failed site reads as down"
        );
        assert_eq!(
            app.state.search.source_counts.get(&SourceId::CineVault),
            Some(&3),

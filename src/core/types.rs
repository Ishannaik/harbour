...
impl SourceId {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            SourceId::TpbTv => "tpb-tv",
            SourceId::X1337Movies => "x1337-movies",
            SourceId::X1337Tv => "x1337-tv",
            SourceId::Eztv => "eztv",
            SourceId::Nyaa => "nyaa",
...
pub struct TorrentResult {
    ...
    ///
    /// Sources that hide the magnet behind a detail page (1337x, FitGirl,
    /// BitTorrented) would otherwise have to fetch one extra page *per row* at
...
mod tests {
    ...
    fn source_id_serde_uses_the_wire_form_not_the_variant_name() {
        let json = serde_json::to_string(&SourceId::TpbMovies).expect("serialize");
        assert_eq!(json, "\"tpb-movies\"");
        let back: SourceId = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(back, SourceId::TpbMovies);
    }
    ...
    fn completed_item_is_derived_from_the_ledger_not_stored_twice() {
        ...
        assert_eq!(done.source, Some(SourceId::Yts));
    }

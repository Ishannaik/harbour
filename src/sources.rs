...
mod tests {
    ...
    async fn wait_for_cinevault_results(
        ...
        let body = format!(
            r#"{{"results":[
                ...
                {{"info_hash":"fedcba9876543210fedcba9876543210fedcba98","name":"   ",
                  "size_bytes":1,"seeders":1,"leechers":1,"source":"tsukibase"}}
            ]}}"#
        );
        let (base, handle) = spawn_indexer(vec![(200, body)]);
        let source = HttpSource::new(base);
    ...
    async fn the_sources_array_populates_the_health_store() {
        ...
        let body = r#"{
            "results": [],
            "sources": [
                {"id": "cinevault", "status": "online", "count": 3},
                {"id": "tsukibase", "status": "empty", "count": 0},
                {"id": "gameshub", "status": "offline", "count": 0}
            ]
        }"#
        .to_string();
        let (base, handle) = spawn_indexer(vec![(200, body)]);
    ...
    async fn a_malformed_sources_entry_is_skipped_not_fatal() {
        ...
        let body = r#"{
            ...
            "sources": [
                ...
                {"id": "cinevault", "status": "flying", "count": 1},
                {"id": "tsukibase", "status": "empty", "count": "lots"},
                {"id": "gameshub", "status": "offline", "count": 0}
            ]
        }"#
        .to_string();
    ...
    async fn an_answer_without_sources_keeps_the_previous_report() {
        ...
        source
            ...
            .expect("search");
        assert_eq!(
            source.reported_status().get(&SourceId::CineVault),
            Some(&(SourceStatus::Online, 3)),
    ...
    async fn a_new_search_clears_stale_site_health() {
        ...
        // A one-line NDJSON stream: only cinevault answers, then the connection closes.
        let server = std::thread::spawn(move || {
            ...
            let _ = stream.write_all(
                b"{\"source\":\"cinevault\",\"status\":\"online\",\"count\":1,\"results\":[]}\n",
            );
            let _ = stream.flush();
        });

        let source = HttpSource::new(base);
        // A previous query reported tsukibase Empty and gameshub Offline.
        source.record_one(SourceId::TsukiBase, SourceStatus::Empty, 0);
        source.record_one(SourceId::GamesHub, SourceStatus::Offline, 0);
    ...
    async fn a_cancelled_stream_stops_at_the_next_line() {
        ...
        // An NDJSON stream held open: cinevault answers, then — 500ms later, after
        // the test has cancelled — tsukibase is sent but must never reach the UI.
        let server = std::thread::spawn(move || {
            let (mut stream, _peer) = listener.accept().expect("accept");
            let _req = read_request(&mut stream);
            let _ = write!(
            ...
            let _ = stream.flush();
            // Give the test time to observe cinevault and cancel before the next line.
            std::thread::sleep(std::time::Duration::from_millis(500));
            let _ = stream.write_all(
                b"{\"source\":\"tsukibase\",\"status\":\"offline\",\"count\":0,\"results\":[]}\n",
            );
            let _ = stream.flush();
        ...
        // Wait for cinevault to land, then cancel — tsukibase is 500ms behind.
        let saw_cinevault = wait_for_cinevault_results(&mut rx, Duration::from_secs(5)).await;
        assert!(saw_cinevault, "the first chunk's results must arrive");
        cancel.cancel();
        handle.await.expect("search task");

        // Well past the 500ms gap, so a leaked tsukibase line would have arrived.

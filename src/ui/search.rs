...
const SIDEBAR: &[(SourceGroup, &[(SourceId, &str)])] = &[
    ...
    (
        ...
        &[
            ...
            (SourceId::AnimeMirror, "AnimeMirror"),
        ],
    ),
];
...
mod tests {
    ...
    fn seeder_and_leecher_tiers_color_by_swarm_size() {
        ...
        // CineVault reports swarm health, so a real 0 is the digit 0, dim.
    ...
    fn unreported_health_is_an_em_dash_never_a_guessed_band() {
        // FR-24: GamesHub/FanSubs carry no swarm counts. seeders: 0 means
        // unknown — an em dash — never a made-up 50+/10+ band.
        let theme = Theme::titanium();
        let state = SearchState::default();
        for source in [SourceId::GamesHub, SourceId::FanSubs] {
    ...
    fn reported_zero_swarm_renders_zero() {
        let theme = Theme::titanium();
        let state = SearchState::default();
        for source in [SourceId::CineVault, SourceId::ShowPort] {
            let row = TorrentResult {
                seeders: 0,
                leechers: 0,
                source,
    ...
    fn source_status_text(health: SourceStatus, count: usize) -> String {
        let mut state = SearchState::default();
        state.source_health.insert(SourceId::CineVault, health);
        if count > 0 {
            state.source_counts.insert(SourceId::CineVault, count);
        }
